from .base import BasePRNG
import time
import hashlib
import struct

# S-box
SBOX = [
    
]

CK = [
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
]

FK = [0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc]

def rotl(x, n):
    return ((x << n) & 0xffffffff) | (x >> (32 - n))

def tau(a):
    b0 = SBOX[(a >> 24) & 0xff]
    b1 = SBOX[(a >> 16) & 0xff]
    b2 = SBOX[(a >> 8) & 0xff]
    b3 = SBOX[a & 0xff]
    return (b0 << 24) | (b1 << 16) | (b2 << 8) | b3

def L_prime(b):
    return b ^ rotl(b, 13) ^ rotl(b, 23)

def T_prime(a):
    return L_prime(tau(a))

def key_expansion(mk):
    k = [0] * 36
    mk0, mk1, mk2, mk3 = struct.unpack(">4I", mk)
    k[0] = mk0 ^ FK[0]
    k[1] = mk1 ^ FK[1]
    k[2] = mk2 ^ FK[2]
    k[3] = mk3 ^ FK[3]
    
    rk = []
    for i in range(32):
        k[i+4] = k[i] ^ T_prime(k[i+1] ^ k[i+2] ^ k[i+3] ^ CK[i])
        rk.append(k[i+4])
    return rk

def L(b):
    return b ^ rotl(b, 2) ^ rotl(b, 10) ^ rotl(b, 18) ^ rotl(b, 24)

def T(a):
    return L(tau(a))

class SM4:
    def __init__(self, key):
        self.rk = key_expansion(key)

    def encrypt(self, block):
        x = list(struct.unpack(">4I", block))
        for i in range(32):
            x[i % 4] = x[i % 4] ^ T(x[(i+1)%4] ^ x[(i+2)%4] ^ x[(i+3)%4] ^ self.rk[i])
            
        # Reverse for output
        return struct.pack(">4I", x[3], x[2], x[1], x[0])

class SM4Generator(BasePRNG):
    def __init__(self, seed=None):
        super().__init__(seed)
        if seed is None:
            seed = int(time.time())
            
        # Derive 128-bit key and 128-bit IV
        key_hash = hashlib.md5(str(seed).encode()).digest()
        self.key = key_hash # 16 bytes
        
        iv_hash = hashlib.md5((str(seed) + "IV").encode()).digest()
        self.iv = int.from_bytes(iv_hash, 'big')
        
        self.cipher = SM4(self.key)
        self.counter = self.iv

    def generate(self, length_bits):
        out_bytes = bytearray()
        bytes_needed = (length_bits + 7) // 8
        
        while len(out_bytes) < bytes_needed:
            counter_bytes = self.counter.to_bytes(16, 'big')
            block = self.cipher.encrypt(counter_bytes)
            out_bytes.extend(block)
            self.counter = (self.counter + 1) % (2**128)
            
        bits = "".join(f"{b:08b}" for b in out_bytes)
        return bits[:length_bits]
